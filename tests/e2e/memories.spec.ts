...
test.describe('Memory Settings Page - Facts Tab', () => {
  test.beforeEach(async ({ page }) => {
    await signInWithClerk(page);
    await clearActiveOrg(page);
  });
  ...
  test('Can view and switch between Memories and Facts tabs', async ({ page }) => {
    ...
    const memoriesTab = page.locator('[data-testid="tab-memories"]');
    const factsTab = page.locator('[data-testid="tab-facts"]');

    await expect(memoriesTab.or(factsTab).first()).toBeVisible({ timeout: DEFAULT_TIMEOUT });

    // If Facts tab exists, click it
    if (await factsTab.isVisible({ timeout: 3000 }).catch(() => false)) {
      await factsTab.click();
      await page.waitForTimeout(500);

      // Should see Facts-related content
      const factsContent = page.locator('text=/stored locally in your browser|Facts are stored/i');
      await expect(factsContent.first()).toBeVisible({ timeout: 5000 });
      console.log('✅ Facts tab is working');
    }

    // Click back to Memories tab
    if (await memoriesTab.isVisible({ timeout: 3000 }).catch(() => false)) {
      await memoriesTab.click();
      await page.waitForTimeout(500);
      console.log('✅ Memories tab is working');
    }
  });
  ...
  test('Facts list shows client-side extracted facts', async ({ page }) => {
    ...
    // First have a conversation to potentially extract facts
    const response = await sendMessageAndWaitForResponse(
      page,
    ...
    // Switch to Facts tab
    const factsTab = page.locator('[data-testid="tab-facts"]');
    if (await factsTab.isVisible({ timeout: 5000 }).catch(() => false)) {
      await factsTab.click();
      await page.waitForTimeout(1000);

      // Take screenshot
      await page.screenshot({ path: 'test-results/facts-page.png', fullPage: true });

      // Check for facts list or empty state
      const factsList = page.locator('[data-testid="fact-list"]');
      const emptyState = page.locator('[data-testid="fact-empty"]');
      const loadingState = page.locator('[data-testid="fact-loading"]');

      await expect(
        factsList.or(emptyState).or(loadingState).first()
      ).toBeVisible({ timeout: 10000 });

      console.log('✅ Facts tab shows content correctly');
    }
  });
});
